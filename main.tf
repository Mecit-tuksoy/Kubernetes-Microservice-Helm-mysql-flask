...
resource "github_repository" "myrepo" {
  name = "microservice-kubernetes-mysql-flask"
  visibility = "public"
  auto_init = true
}
...
variable "files" {
  default = ["mysql_secret-ConfigMap.yaml", "pv-pvc.yaml", "mysql_deployement-service.yaml", "result_server_deployment-service.yml", "web_server_deployment-service.yaml"]
}
...
resource "aws_instance" "master" {
  ...
  user_data            = templatefile("master.sh", { user-data-git-token = var.git-token, user-data-git-name = var.git-name, repo-name = github_repository.myrepo.name })
